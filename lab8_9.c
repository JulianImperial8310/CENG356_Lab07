...
unsigned int CPU_fetchCode(char *mem, int codeOffset){
    ...
    code = ((unsigned int)(unsigned char)mem[codeOffset] << 24) |
           ((unsigned int)(unsigned char)mem[codeOffset + 1] << 16) |
           ((unsigned int)(unsigned char)mem[codeOffset + 2] << 8) |
           ((unsigned int)(unsigned char)mem[codeOffset + 3]);
...
opcode = (machineCode >> 26) & 0x3f; //The Top 6 bits
if (opcode == 0){
    opcode = machineCode & 0x3F; // Last 6 Bits
}
